...
The current EPUB reference includes the text map and native `glossref` markup. It does not demonstrate every WebPub sidecar. Do not infer sidecar parity between formats. Word-timing data is not part of this public V1 contract; use native EPUB Media Overlays when they are present, and otherwise keep narration at node level.
...
When a new public format or capability is added, include a publishable fixture package and expected-results matrix with it. PNLD references remain LIP-only and are not part of this public contract.
